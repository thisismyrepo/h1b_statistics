...
NumberOfRows = round(len(fields)/53)
NumberOfCols = 53
length_of_fields=len(fields)

# Expected output: TOP_OCCUPATIONS;NUMBER_CERTIFIED_APPLICATIONS;PERCENTAGE
# JOB_TITLE at indices 23 (title), 76, 129, 182, 235, 288, 341, 394, 447, 500
# CASE_STATUS at indices 3 (title), 56, 109, 162, 215, 268, 321, 374, 427, 480


#outputfile = open('./output/h1b_output.txt','w') 

# get list of occupations
i=75
j=0
occupation_list=list()
occupation_indices=list()
while i<=length_of_fields:
    occupation_list.append(fields[i])
    occupation_indices.append(i)
    i=i+53
    j=j+1
    
    unique_occupation_list = list()

     
 # traverse for all elements
for x in occupation_list:
        # check if exists in unique_occupation_list or not
   if x not in unique_occupation_list:
       unique_occupation_list.append(x)

i=0
j=0
number_of_unique_occupations=len(unique_occupation_list)
unique_state_list=[0]*number_of_unique_occupations
number_of_certified_apps = [0]*number_of_unique_occupations

total_certified_apps=NumberOfRows-1



while j<number_of_unique_occupations:
    while i<len(occupation_list):   
        if occupation_list[i]==unique_occupation_list[j]:
 #           occupation_index=state_indices[i]
#            state_name=int(fields[occupation_index-20])
#            unique_cost_list[j]=int(unique_cost_list[j])+cost_of_drug
            number_of_certified_apps[j]=number_of_certified_apps[j]+1
        i=i+1
    j=j+1
#   
# print output values
counter=0
#outputfile.write("TOP_OCCUPATIONS;NUMBER_CERTIFIED_APPLICATIONS;PERCENTAGE \n")
print("TOP_OCCUPATIONS;NUMBER_CERTIFIED_APPLICATIONS;PERCENTAGE \n")
while counter<number_of_unique_occupations:
    percentage=(number_of_certified_apps[counter]/total_certified_apps)*100
    #outputfile.write(unique_occupation_list[counter],',', number_of_certified_apps[counter],',', unique_cost_list[counter])
    print(unique_occupation_list[counter],',', number_of_certified_apps[counter],',', percentage,'%')
    print("\n")
    counter=counter+1
